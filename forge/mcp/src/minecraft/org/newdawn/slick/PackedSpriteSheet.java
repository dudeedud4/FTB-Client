...
/**
 * A sprite sheet packed and defined by the Pacific Software Image Packer
 * available from:
 * 
...
public class PackedSpriteSheet {
	...
	private Image image;
	/**
	 * The base path where the image is expected to be found based on the
	 ...
	 *             Indicates a failure to read the definition file
	 */
	...
	public Image getSprite(String name) {
		...
		if (section == null) {
			throw new RuntimeException("Unknown sprite from packed sheet: "
					+ name);
		}

		return image.getSubImage(section.x, section.y, section.width,
				section.height);
	}
	...
	public SpriteSheet getSpriteSheet(String name) {
		...
		return new SpriteSheet(image, section.width / section.tilesx,
				section.height / section.tilesy);
	}
	 ...
	 * 
	 * @param def
	 *            The location of the definitions file
	 * @param trans
	 *            The color to be treated as transparent
	 * @throws SlickException
	 *             Indicates a failure to read or parse the definitions file or
	 *             referenced image.
	 */
	private void loadDefinition(String def, Color trans) throws SlickException {
		BufferedReader reader = new BufferedReader(new InputStreamReader(
				ResourceLoader.getResourceAsStream(def)));

		try {
			image = new Image(basePath + reader.readLine(), false, filter,
					trans);
			while (reader.ready()) {
	...
	private class Section {
		 ...
		 * 
		 * @param reader
		 *            The reader from which the definition can be read
		 * @throws IOException
		 *             Indicates a failure toread the provided stream
		 */
