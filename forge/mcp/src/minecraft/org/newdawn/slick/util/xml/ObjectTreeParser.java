 ...
 * provide the same functionality as JAXB or the variety of XML bindings out
 * there. This is a utility to map XML onto an existing data model. The idea
 * being that the design level model should not be driven by the XML schema
 * thats defined. The two arn't always equal and often you end up with a set of
 * class that represent your XML that you then have to traverse to extract into
 * your normal data model.
 * 
 * This utility hopes to take a piece of XML and map it onto a previously
 * designed data model. At the moment it's way to tied to the structure of the
 * XML but this will hopefully change with time.
 * 
 * XML element names must be mapped to class names. This can be done in two ways
 * either:
 * 
 * - Specify an explict mapping with addElementMapping() - Specify the default
 * package name and use the element name as the class name
 * 
 * Each attribute in an element is mapped into a property of the element class,
 * preferably through a set<AttrName> bean method, but alternatively by direct
 * injection into private fields.
 * 
 * Each child element is added to the target class by call the method add() on
 * it with a single parameter of the type generated for the child element.
 * 
 * Classes can optionally implement setXMLElementName(String) and
 * setXMLElementContent(String) to recieve the name and content respectively of
 * the XMLElement they were parsed from. This can help when mapping two elements
 * to a single class.
 * 
 * To reiterate, I'm not sure this is a good idea yet. It helps me as a utility
 * since I've done this several times in the past but in the general case it may
 * not be perfect. Consider a custom parser using XMLParser or JAXB (et al)
 * seriously instead.
 * 
 * @author kevin
 * 
...
public class ObjectTreeParser {
	...
	/**
	 * Create an object tree parser specifing the default package where classes
	 * will be search for using the XML element name
	 * 
	 ...
	 * This is sometimes useful to not clash with the existing data model
	 * methods.
	 * 
	 * @param methodName
	 ...
	 *             Indicates a failure to parse the XML, most likely the XML is
	...
	/**
	 * Deterine the name of the class that should be used for a given XML
	 * element name.
	 * 
	 * @param name
	 *            The name of the XML element
	...
	/**
	 * Traverse the XML element specified generating the appropriate object
	 * structure for it and it's children
	 * 
	 * @param current
	...
	private Object traverse(XMLElement current, Object instance)
		...
		if (clazz == null) {
			throw new SlickXMLException("Unable to map element " + name
					+ " to a class, define the mapping");
		}
		...
		try {
			if (instance == null) {
				...
				Method elementNameMethod = getMethod(clazz,
						"setXMLElementName", new Class[] { String.class });
				if (elementNameMethod != null) {
					invoke(elementNameMethod, instance, new Object[] { name });
				}
				Method contentMethod = getMethod(clazz, "setXMLElementContent",
						new Class[] { String.class });
				if (contentMethod != null) {
					invoke(contentMethod, instance,
							new Object[] { current.getContent() });
				}
			}
			...
			for (int i = 0; i < attrs.length; i++) {
				...
				if (method == null) {
					...
					if (field != null) {
						...
						Log.info("Unable to find property on: " + clazz
								+ " for attribute: " + attrs[i]);
					}
				} else {
					String value = current.getAttribute(attrs[i]);
					Object typedValue = typeValue(value,
			...
			for (int i = 0; i < children.size(); i++) {
				...
				if (child != null) {
					...
					Method method = findMethod(clazz, methodName,
							child.getClass());
					if (method == null) {
						Log.info("Unable to find method to add: " + child
								+ " to " + clazz);
					} else {
			...
			throw new SlickXMLException("Unable to instance " + clazz
					+ " for element " + name
					+ ", no zero parameter constructor?", e);
	 ...
	 * @throws SlickXMLException
	 *             Indicates there is no automatic way of converting the value
	 *             to the type
	 */
	private Object typeValue(String value, Class clazz)
		...
		try {
			...
			throw new SlickXMLException("Failed to convert: " + value
	...
	/**
	 * Find a field in a class by it's name. Note that this method is only
	 * needed because the general reflection method is case sensitive
	 * 
	 * @param clazz
	 *            The clazz to search
	 * @param name
	...
	/**
	 * Find a method in a class by it's name. Note that this method is only
	 * needed because the general reflection method is case sensitive
	 * 
	 * @param clazz
	 *            The clazz to search
	 ...
	 *            The value to set
	 * @throws SlickXMLException
	 *             Indicates a failure to set or access the field
	 */
	private void setField(Field field, Object instance, Object value)
		...
		try {
			...
			throw new SlickXMLException("Failed to set: " + field
					+ " for an XML attribute, is it valid?", e);
		} catch (IllegalAccessException e) {
			throw new SlickXMLException("Failed to set: " + field
					+ " for an XML attribute, is it valid?", e);
		} finally {
	 ...
	 * @throws SlickXMLException
	 *             Indicates a failure to call or access the method
	 */
	private void invoke(Method method, Object instance, Object[] params)
		...
		try {
			...
			throw new SlickXMLException("Failed to invoke: " + method
					+ " for an XML attribute, is it valid?", e);
		} catch (IllegalAccessException e) {
			throw new SlickXMLException("Failed to invoke: " + method
					+ " for an XML attribute, is it valid?", e);
	...
	/**
	 * Get a method on a given class. Only here for tidy purposes, hides the the
	 * big exceptions.
	 * 
	 * @param clazz
	 *            The class to search
