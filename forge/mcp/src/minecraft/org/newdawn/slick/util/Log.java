...
public final class Log {
	...
	/**
	 * true if activated by the system property
	 * "org.newdawn.slick.forceVerboseLog"
	 */
	private static boolean forcedVerbose = false;

	/**
	 * The debug property which can be set via JNLP or startup parameter to
	 * switch logging mode to verbose for games that were released without
	 * verbose logging value must be "true"
	 */
	...
	/**
	 * Indicate that we want verbose logging. The call is ignored if verbose
	 * logging is forced by the system property
	 * "org.newdawn.slick.forceVerboseLog"
	 * 
	 * @param v
		...
		try {
			AccessController.doPrivileged(new PrivilegedAction() {
				public Object run() {
					...
					if ((val != null)
							&& (val.equalsIgnoreCase(Log.forceVerbosePropertyOnValue))) {
	 ...
	 * Only be called when system property "org.newdawn.slick.forceVerboseLog"
	 * is set to true. You must not call this method directly.
	 */
