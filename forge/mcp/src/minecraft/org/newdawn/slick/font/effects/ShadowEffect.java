...
public class ShadowEffect implements ConfigurableEffect {
	 ...
	 * @param xDistance
	 *            The distance from the text on the x axis the shadow should be
	 *            rendered
	 * @param yDistance
	 *            The distance from the text on the y axis the shadow should be
	 *            rendered
	 * @param opacity
	 *            The transparency factor of the shadow
	 */
	...
	public void draw(BufferedImage image, Graphics2D g,
		...
		for (Iterator iter = unicodeFont.getEffects().iterator(); iter
			...
			if (effect instanceof OutlineEffect) {
				...
				g.setComposite(AlphaComposite.Src); // Prevent shadow and
													// outline shadow alpha from
													// combining.
		...
		g.dispose();
		if (blurKernelSize > 1 && blurKernelSize < NUM_KERNELS
				&& blurPasses > 0)
			blur(image);
	}
	...
	/**
	 * Get the distance on the X axis from the text the shadow should be
	 * generated at
	 * 
	...
	/**
	 * Sets the pixels to offset the shadow on the x axis. The glyphs will need
	...
	/**
	 * Get the distance on the Y axis from the text the shadow should be
	 * generated at
	 * 
	...
	/**
	 * Sets how many neighboring pixels are used to blur the shadow. Set to 0
	 * for no blur.
	 * 
	...
	/**
	 * Sets the number of times to apply a blur to the shadow. Set to 0 for no
	 * blur.
	 * 
	 * @param blurPasses
	...
	public List getValues() {
		...
		values.add(EffectUtil
				.floatValue(
						...
						Float.MIN_VALUE,
						Float.MAX_VALUE,
						"This setting is the amount of pixels to offset the shadow on the"
	...
	/**
	 * Generate the blur kernels which will be repeatedly applied when blurring
	 * images
	 * 
	 * @param level
