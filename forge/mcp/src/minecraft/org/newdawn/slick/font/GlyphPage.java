...
public class GlyphPage {
	...
	private static ByteBuffer scratchByteBuffer = ByteBuffer
	...
	private static BufferedImage scratchImage = new BufferedImage(
			MAX_GLYPH_SIZE, MAX_GLYPH_SIZE, BufferedImage.TYPE_INT_ARGB);
	/** The graphics context form the temporary image */
	private static Graphics2D scratchGraphics = (Graphics2D) scratchImage
			.getGraphics();

	static {
		scratchGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_ON);
		scratchGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
				RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
		scratchGraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS,
	 ...
	 *             if the backing texture could not be created.
	 */
	 ...
	 *            This is the maximum number of glyphs to load from the list.
	 *            Set to -1 to attempt to load all the glyphs.
	 * @return The number of glyphs that were actually loaded.
	 * @throws SlickException
	 *             if the glyph could not be rendered.
	 */
	public int loadGlyphs(List glyphs, int maxGlyphsToLoad)
			throws SlickException {
		if (rowHeight != 0 && maxGlyphsToLoad == -1) {
			// If this page has glyphs and we are not loading incrementally,
			// return zero if any of the glyphs don't fit.
			int testX = pageX;
			int testY = pageY;
		...
		for (Iterator iter = getIterator(glyphs); iter.hasNext();) {
			...
			if (i == maxGlyphsToLoad) {
				// If loading incrementally, flip orderAscending so it won't
				// change, since we'll probably load the rest next time.
		...
		// Every other batch of glyphs added to a page are sorted the opposite
		// way to attempt to keep same size glyps together.
		orderAscending = !orderAscending;
	...
	private void renderGlyph(Glyph glyph, int width, int height)
		...
		for (Iterator iter = unicodeFont.getEffects().iterator(); iter
				.hasNext();)
			((Effect) iter.next()).draw(scratchImage, scratchGraphics,
					unicodeFont, glyph);
		...
		GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, pageX, pageY, width,
				height, GL12.GL_BGRA, GL11.GL_UNSIGNED_BYTE, scratchByteBuffer);
	...
	/**
	 * Returns an iterator for the specified glyphs, sorted either ascending or
	 * descending.
	 * 
	 * @param glyphs
