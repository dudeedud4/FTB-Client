 ...
 * the one we're transitioning from. The second state is specified in the
 * constructor.
 * 
 * By default one state will simply be rendered over the other. Subclass this
 * transition overriding the preRenderFirstState and preRenderSecondState to
 * setup the rendering for each state (alpha or what ever). Note that it's also
 * possible to use the postRenderSecondState method to clean up your OpenGL
 * setup.
 * 
 * So these methods are called like so:
 * 
 * preRenderFirstState() = the first state is rendered preRenderSecondState() =
 * the second state is rendered postRenderSecondState()
 * 
 * @author kevin
 */
public abstract class CrossStateTransition implements Transition {
	 ...
	 * 
	 * @param secondState
	 *            The secondary state with combining with the source state.
	 */
	...
	/**
	 * Notification that the transition is about to render the first state is
	 ...
	 * @throws SlickException
	 *             Indicates a failure to setup the rendering state - throw for
	 *             anything that goes wrong
	 */
	public void preRenderFirstState(StateBasedGame game,
			GameContainer container, Graphics g) throws SlickException {
	}

	/**
	 * Notification that the transition is about to render the second state is
	 * the cross transition.
	 * 
	 * @param game
	 *            The game being rendered
	...
	/**
	 * Notification that the transition is has just rendered the second state is
	 * the cross transition.
	 * 
	 * @param game
	 *            The game being rendered
