...
/**
 * A piece of music loaded and playable within the game. Only one piece of music
 * can play at any given time and a channel is reserved so music will always
 * play.
 * 
 * @author kevin
...
public class Music {
	...
	/**
	 * Poll the state of the current music. This causes streaming music to
	 * stream and checks listeners. Note that if you're using a game container
	 * this will be auto-magically called for you.
	 * 
	 * @param delta
	 *            The amount of time since last poll
	 */
	...
	/**
	 * True if the music is being repositioned and it is therefore normal that
	 * it's not playing
	 */
	 ...
	 *            The location of the music
	 * @param streamingHint
	 *            A hint to indicate whether streaming should be used if
	 *            possible
	 * @throws SlickException
	 */
	 ...
	 * 
	 * @param newMusic
	 *            The new music that will be played
	 */
	 ...
	 *            The volume to play the music at (1.0 = default)
	 * @param loop
	 *            if false the music is played once, the music is looped
	 *            otherwise
	 */
	 ...
	 * 
	 * @param volume
	 *            The volume to play music at. 0 - 1, 1 is Max
	 */
	 ...
	 * 
	 * @return The volume of this music, still effected by global SoundStore
	 *         volume. 0 - 1, 1 is Max
	 */
	 ...
	 * 
	 * @param delta
	 *            The amount of time in milliseconds thats passed since last
	 *            update
	 */
	void update(int delta) {
		...
		if (fadeTime > 0) {
			...
			float offset = (fadeEndGain - fadeStartGain)
					* (1 - (fadeTime / (float) fadeDuration));
			setVolume(fadeStartGain + offset);
		}
	}

	/**
	 * Seeks to a position in the music. For streaming music, seeking before the
	 * current position causes the stream to be reloaded.
	 * 
	 * @param position
	 *            Position in seconds.
