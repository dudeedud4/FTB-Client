...
public class Image implements Renderable {
	...
	public Image(String ref, boolean flipped, int f, Color transparent)
		...
		try {
			...
			texture = InternalTextureLoader.get().getTexture(ref, flipped,
					filter, trans);
		} catch (IOException e) {
			Log.error(e);
	...
	/**
	 * Set the image filtering to be used. This will cause the image to be
	 * reloaded.
	 * 
	 * @param f
	 *            The filtering mode to use
	 * @throws SlickException
	 *             Indicates a failure to revalidate the image source
	 */
	 ...
	 *            The height of the image
	 * @throws SlickException
	 *             Indicates a failure to create the underlying resource
	 */
	public Image(int width, int height) throws SlickException {
		...
		try {
			...
			Log.error(e);
			throw new SlickException("Failed to create empty image " + width
					+ "x" + height);
		}
	 ...
	 * Note that this can be null in the cases where an image was
	...
	/**
	 * Give this image a meaningful tagging name. Can be used as user
	 * data/identifier for the image.
	 * 
	 * @param name
	 ...
	 * @throws SlickException
	 *             Indicates a failure to create a graphics context
	 */
	 ...
	 *             Indicates a failure to load the image
	 */
	private void load(InputStream in, String ref, boolean flipped, int f,
			Color transparent) throws SlickException {
		this.filter = f == FILTER_LINEAR ? SGL.GL_LINEAR : SGL.GL_NEAREST;
		...
		try {
			...
			texture = InternalTextureLoader.get().getTexture(in, ref, flipped,
					filter, trans);
		} catch (IOException e) {
			Log.error(e);
			throw new SlickException("Failed to load image from: " + ref, e);
	 ...
	 * 
	 * @param x
	 *            The x coordinate of center of rotation relative to the top
	 *            left corner of the image
	 * @param y
	 *            The y coordinate of center of rotation relative to the top
	 *            left corner of the image
	 */
	...
	public void drawFlash(float x, float y, float width, float height, Color col) {
		...
		if (GLContext.getCapabilities().GL_EXT_secondary_color) {
			...
			EXTSecondaryColor.glSecondaryColor3ubEXT((byte) (col.r * 255),
					(byte) (col.g * 255), (byte) (col.b * 255));
		}

		GL.glTexEnvi(SGL.GL_TEXTURE_ENV, SGL.GL_TEXTURE_ENV_MODE,
				SGL.GL_MODULATE);
	...
	/**
	 * Set the angle to rotate this image to. The angle will be normalized to be
	 * 0 <= angle < 360. The image will be rotated around its center.
	 * 
	 * @param angle
	 *            The angle to be set
	 */
	...
	/**
	 * Get the current angle of rotation for this image. The image will be
	...
	/**
	 * Add the angle provided to the current rotation. The angle will be
	 * normalized to be 0 <= angle < 360. The image will be rotated around its
	 * center.
	 * 
	 * @param angle
	 *            The angle to add.
	...
	/**
	 * Get a sub-part of this image. Note that the create image retains a
	 * reference to the image data so should anything change it will affect
	 * sub-images too.
	 * 
	 * @param x
	...
	public Image getSubImage(int x, int y, int width, int height) {
		...
		float newTextureOffsetX = ((x / (float) this.width) * textureWidth)
				+ textureOffsetX;
		float newTextureOffsetY = ((y / (float) this.height) * textureHeight)
				+ textureOffsetY;
	...
	/**
	 * Draw a section of this image at a particular location and scale on the
	 * screen
	 * 
	 * @param x
	 ...
	 *            draw from this image (i.e. relative to this image)
	 * @param srcy2
	 *            The t position of the bottom right cornder of rectangle to
	 *            draw from this image (i.e. relative to this image)
	 */
	public void draw(float x, float y, float srcx, float srcy, float srcx2,
			float srcy2) {
	...
	/**
	 * Draw a section of this image at a particular location and scale on the
	 * screen, while this is image is "in use", i.e. between calls to startUse
	 * and endUse.
	 * 
	 * @param x
	 *            The x position to draw the image
	...
	/**
	 * Get a copy of this image. This is a shallow copy and does not duplicate
	 * image adata.
	 * 
	...
	public void endUse() {
		if (inUse != this) {
			throw new RuntimeException(
					"The sprite sheet is not currently in use");
		}
		inUse = null;
		GL.glEnd();
	}
	...
	/**
	 * Start using this sheet. This method can be used for optimal rendering of
	 * a collection of sprites from a single sprite sheet. First, startUse().
	 * Then render each sprite by calling renderInUse(). Finally, endUse().
	 * Between start and end there can be no rendering of other sprites since
	 * the rendering is locked for this sprite sheet.
	 */
	...
	public String toString() {
		...
		return "[Image " + ref + " " + width + "x" + height + "  "
				+ textureOffsetX + "," + textureOffsetY + "," + textureWidth
				+ "," + textureHeight + "]";
	}
	...
	public Color getColor(int x, int y) {
		...
		if (texture.hasAlpha()) {
			return new Color(translate(pixelData[offset]),
					translate(pixelData[offset + 1]),
					translate(pixelData[offset + 2]),
					translate(pixelData[offset + 3]));
		} else {
			return new Color(translate(pixelData[offset]),
					translate(pixelData[offset + 1]),
					translate(pixelData[offset + 2]));
	...
	/**
	 * Destroy the image and release any native resources. Calls on a destroyed
	 * image have undefined results
	 * 
	 * @throws SlickException
	 *             Indicates a failure to release resources on the graphics card
	 */
