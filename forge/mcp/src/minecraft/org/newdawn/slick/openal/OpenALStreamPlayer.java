...
/**
 * A generic tool to work on a supplied stream, pulling out PCM data and
 * buffered it to OpenAL as required.
 * 
 * @author Kevin Glass
...
public class OpenALStreamPlayer {
	...
	/**
	 * The buffer holding the names of the OpenAL buffer thats been fully played
	 * back
	 */
	 ...
	 * @param ref
	 *            A reference to the audio file to stream
	 */
	 ...
	 * 
	 * @throws IOException
	 *             Indicates a failure to open the underling resource
	 */
	...
	/**
	 * Check if the playback is complete. Note this will never return true if
	 * we're looping
	 * 
	...
	/**
	 * Poll the bufferNames - check if we need to fill the bufferNames with
	 * another section.
	 * 
	...
	public void update() {
		...
		while (processed > 0) {
			...
			float bufferLength = (AL10.alGetBufferi(bufferIndex, AL10.AL_SIZE) / sampleSize)
					/ sampleRate;
	...
	public boolean stream(int bufferId) {
		try {
			...
			if (count != -1) {
				...
				int format = audio.getChannels() > 1 ? AL10.AL_FORMAT_STEREO16
						: AL10.AL_FORMAT_MONO16;
				try {
					AL10.alBufferData(bufferId, format, bufferData,
							audio.getRate());
				} catch (OpenALException e) {
					Log.error("Failed to loop buffer: " + bufferId + " "
							+ format + " " + count + " " + audio.getRate(), e);
					return false;
				}
			} else {
				if (loop) {
