...
public class SoundStore {
	...
	int playAsSoundAt(int buffer, float pitch, float gain, boolean loop,
		...
		if (soundWorks) {
			if (sounds) {
				...
				sourcePos.flip();
				sourceVel.flip();
				AL10.alSource(sources.get(nextSource), AL10.AL_POSITION,
						sourcePos);
				AL10.alSource(sources.get(nextSource), AL10.AL_VELOCITY,
						sourceVel);
	...
	/**
	 * Check if the supplied player is currently being polled by this sound
	 * store.
	 * 
	 * @param player
	 *            The player to check
	...
	public Audio getOgg(String ref, InputStream in) throws IOException {
		...
		if (loaded.get(ref) != null) {
			...
			try {
				...
				Log.error(e);
				Sys.alert("Error",
						"Failed to load: " + ref + " - " + e.getMessage());
				throw new IOException("Unable to load: " + ref);
			}
		}
	 ...
	 * 
	 * @param id
	 *            The ID of the underlying OpenAL source as returned from
	 *            playAsSoundEffect
	 */
