...
public class AppGameContainer extends GameContainer {
	...
	public void setMouseCursor(Image image, int hotSpotX, int hotSpotY)
		...
		try {
			...
			ByteBuffer buffer = BufferUtils.createByteBuffer(temp.getWidth()
					* temp.getHeight() * 4);
			...
			Cursor cursor = CursorLoader.get().getCursor(buffer, hotSpotX,
					hotSpotY, temp.getWidth(), image.getHeight());
			Mouse.setNativeCursor(cursor);
		} catch (Exception e) {
			Log.error("Failed to load and apply cursor.", e);
		}
	...
	public void setIcons(String[] refs) throws SlickException {
		...
		for (int i = 0; i < refs.length; i++) {
			...
			try {
				bufs[i] = data.loadImage(
						ResourceLoader.getResourceAsStream(refs[i]), flip,
						false, null);
			} catch (Exception e) {
				Log.error(e);
