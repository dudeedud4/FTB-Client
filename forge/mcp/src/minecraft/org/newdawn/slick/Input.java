...
public class Input {
	...
	/**
	 * Disables support for controllers. This means the jinput JAR and native
	 * libs are not required.
	 */
	...
	/**
	 * Set the double click interval, the time between the first and second
	 * clicks that should be interpreted as a double click.
	 * 
	 * @param delay
	...
	/**
	 * Add a listener to be notified of input events. This listener will get
	 * events before others that are currently registered
	 * 
	 * @param listener
	 *            The listener to be notified
	 */
	...
	public void removeKeyListener(KeyListener listener) {
		...
		if (!mouseListeners.contains(listener)
				&& !controllerListeners.contains(listener)) {
	...
	public void removeControllerListener(ControllerListener listener) {
		...
		if (!mouseListeners.contains(listener)
				&& !keyListeners.contains(listener)) {
			allListeners.remove(listener);
		}
	}
	...
	/**
	 * Get the character representation of the key identified by the specified
	 * code
	 * 
	 * @param code
	...
	/**
	 * Check if a particular key has been pressed since this method was last
	 * called for the specified key
	 * 
	 * @param code
	 ...
	 * 
	 * @param button
	 *            The button to check for (note that this includes directional
	 *            controls first)
	...
	/**
	 * Clear the state for isControlPressed method. This will reset all controls
	 * to not pressed
	 */
	 ...
	 * resort in all keys returning that they haven't been pressed, until they
	 * are pressed again
	 */
	 ...
	 * resort in all mouse buttons returning that they haven't been pressed,
	...
	public boolean isButtonPressed(int index, int controller) {
		...
		return ((Controller) controllers.get(controller))
	 ...
	 *             Indicates a failure to use the hardware
	 */
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if ((controller.getButtonCount() >= 3)
						&& (controller.getButtonCount() < MAX_BUTTONS)) {
			...
			for (int i = 0; i < controllers.size(); i++) {
				Log.info(i + " : "
						+ ((Controller) controllers.get(i)).getName());
			}
	...
	/**
	 * Hook to allow us to translate any key character into special key codes
	 * for easier use.
	 * 
	 * @param key
	...
	/**
	 * Notification that the mouse has been pressed and hence we should consider
	 * what we're doing with double clicking
	 * 
	 * @param button
	...
	public void poll(int width, int height) {
		...
		while (allStarts.hasNext()) {
			ControlledInputReciever listener = (ControlledInputReciever) allStarts
					.next();
		...
		while (Keyboard.next()) {
			if (Keyboard.getEventKeyState()) {
				int eventKey = resolveEventKey(Keyboard.getEventKey(),
						Keyboard.getEventCharacter());

				keys[eventKey] = Keyboard.getEventCharacter();
				pressed[eventKey] = true;
				nextRepeat[eventKey] = System.currentTimeMillis()
						+ keyRepeatInitial;
			...
			if (Mouse.getEventButton() >= 0) {
				if (Mouse.getEventButtonState()) {
					...
					for (int i = 0; i < mouseListeners.size(); i++) {
						MouseListener listener = (MouseListener) mouseListeners
								.get(i);
						if (listener.isAcceptingInput()) {
							listener.mousePressed(Mouse.getEventButton(),
									pressedX, pressedY);
							if (consumed) {
								break;
							}
					...
					for (int i = 0; i < mouseListeners.size(); i++) {
						MouseListener listener = (MouseListener) mouseListeners
								.get(i);
						if (listener.isAcceptingInput()) {
							listener.mouseReleased(Mouse.getEventButton(),
									releasedX, releasedY);
							if (consumed) {
								break;
							}
				...
				if (Mouse.isGrabbed()) {
					if ((Mouse.getEventDX() != 0) || (Mouse.getEventDY() != 0)) {
						...
						for (int i = 0; i < mouseListeners.size(); i++) {
							...
							if (listener.isAcceptingInput()) {
								if (anyMouseDown()) {
									listener.mouseDragged(0, 0,
											Mouse.getEventDX(),
											-Mouse.getEventDY());
								} else {
									listener.mouseMoved(0, 0,
											Mouse.getEventDX(),
											-Mouse.getEventDY());
								}
		...
		if (!displayActive) {
			...
			if ((lastMouseX != getMouseX()) || (lastMouseY != getMouseY())) {
				...
				for (int i = 0; i < mouseListeners.size(); i++) {
					...
					if (listener.isAcceptingInput()) {
						if (anyMouseDown()) {
							listener.mouseDragged(lastMouseX, lastMouseY,
									getMouseX(), getMouseY());
						} else {
							listener.mouseMoved(lastMouseX, lastMouseY,
									getMouseX(), getMouseY());
						}
						if (consumed) {
							break;
			...
			for (int i = 0; i < 1024; i++) {
				if (pressed[i] && (nextRepeat[i] != 0)) {
					if (System.currentTimeMillis() > nextRepeat[i]) {
						nextRepeat[i] = System.currentTimeMillis()
								+ keyRepeatInterval;
						consumed = false;
						for (int j = 0; j < keyListeners.size(); j++) {
							KeyListener listener = (KeyListener) keyListeners
									.get(j);
		...
		while (all.hasNext()) {
			ControlledInputReciever listener = (ControlledInputReciever) all
					.next();
			listener.inputEnded();
		}
	...
	/**
	 * Enable key repeat for this input context. This will cause keyPressed to
	 * get called repeatedly at a set interval while the key is pressed
	 * 
	 * @param initial
	...
	/**
	 * Enable key repeat for this input context. Uses the system settings for
	...
	private void fireControlPress(int index, int controllerIndex) {
		...
		for (int i = 0; i < controllerListeners.size(); i++) {
			ControllerListener listener = (ControllerListener) controllerListeners
					.get(i);
			if (listener.isAcceptingInput()) {
				switch (index) {
				case LEFT:
				...
				default:
					...
					listener.controllerButtonPressed(controllerIndex,
							(index - BUTTON1) + 1);
					break;
				}
				if (consumed) {
		...
		for (int i = 0; i < controllerListeners.size(); i++) {
			...
			if (listener.isAcceptingInput()) {
				...
				default:
					...
					listener.controllerButtonReleased(controllerIndex,
							(index - BUTTON1) + 1);
					break;
				}
				if (consumed) {
	 ...
	 *            The location at which the button was clicked
	 * @param clickCount
	 *            The number of times the button was clicked (single or double
	 *            click)
	 */
