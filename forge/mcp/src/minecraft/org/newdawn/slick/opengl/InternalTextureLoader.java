...
/**
 * A texture loaded based on many old versions that will load image data from a
 * file and produce OpenGL textures.
 * 
 * @see ImageData
 * 
 * @author kevin
...
public class InternalTextureLoader {
	...
	/**
	 * True if we should only record the request to load in the intention of
	 * loading the texture later
	 * 
	 * @param deferred
	 ...
	 *             Indicates a failure to load the image
	 */
	public Texture getTexture(InputStream in, String resourceName,
			boolean flipped, int filter) throws IOException {
		return getTexture(in, resourceName, flipped, filter, null);
	}
	 ...
	 *             Indicates a failure to load the image
	 */
	public TextureImpl getTexture(InputStream in, String resourceName,
			boolean flipped, int filter, int[] transparent) throws IOException {
		if (deferred) {
		...
		if (transparent != null) {
			resName += ":" + transparent[0] + ":" + transparent[1] + ":"
					+ transparent[2];
		}
		resName += ":" + flipped;
		...
		TextureImpl tex = getTexture(in, resourceName, GL11.GL_TEXTURE_2D,
	...
	private TextureImpl getTexture(InputStream in, String resourceName,
			int target, int magFilter, int minFilter, boolean flipped,
			int[] transparent) throws IOException {
		...
		LoadableImageData imageData = ImageDataFactory
				.getImageDataFor(resourceName);
		textureBuffer = imageData.loadImage(new BufferedInputStream(in),
				flipped, transparent);
	 ...
	 * @throws IOException
	 *             Indicates a failure to create the texture on the graphics
	 *             hardware
	 */
	 ...
	 */
	public Texture createTexture(final int width, final int height,
			final int filter) throws IOException {
	...
	public Texture getTexture(ImageData dataSource, int filter)
		...
		// create the texture ID for this texture
		int textureID = createTextureID();
		TextureImpl texture = new TextureImpl("generated:" + dataSource,
				target, textureID);
