...
public class CompositeImageData implements LoadableImageData {
	 ...
	 *      boolean, boolean, int[])
	 */
	public ByteBuffer loadImage(InputStream is, boolean flipped,
		...
		for (int i = 0; i < sources.size(); i++) {
			...
			try {
				...
				Log.warn(
						sources.get(i).getClass() + " failed to read the data",
						e);
	...
	private void checkPicked() {
		if (picked == null) {
			throw new RuntimeException(
					"Attempt to make use of uninitialised or invalid composite image data");
		}
	}
