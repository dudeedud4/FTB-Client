...
/**
 * A factory to produce an appropriate render to texture graphics context based
 * on current hardware
 * 
 * @author kevin
 */
public class GraphicsFactory {
	...
	/**
	 * Initialise offscreen rendering by checking what buffers are supported by
	 * the card
	 * 
	 * @throws SlickException
	 *             Indicates no buffers are supported
	 */
	private static void init() throws SlickException {
		...
		if (!fbo && !pbuffer && !pbufferRT) {
			throw new SlickException(
					"Your OpenGL card does not support offscreen buffers and hence can't handle the dynamic images required for this application.");
		}

		Log.info("Offscreen Buffers FBO=" + fbo + " PBUFFER=" + pbuffer
				+ " PBUFFERRT=" + pbufferRT);
	}
	...
	private static Graphics createGraphics(Image image) throws SlickException {
		...
		throw new SlickException(
				"Failed to create offscreen buffer even though the card reports it's possible");
	}
}
