...
public class PBufferGraphics extends Graphics {
	...
	public PBufferGraphics(Image image) throws SlickException {
		...
		Log.debug("Creating pbuffer(rtt) " + image.getWidth() + "x"
				+ image.getHeight());
		...
		if ((Pbuffer.getCapabilities() & Pbuffer.RENDER_TEXTURE_SUPPORTED) == 0) {
			throw new SlickException(
					"Your OpenGL card does not support Render-To-Texture and hence can't handle the dynamic images required for this application.");
		}
	...
	private void init() throws SlickException {
		try {
			...
			final RenderTexture rt = new RenderTexture(false, true, false,
