...
public class QuadBasedLineStripRenderer implements LineStripRenderer {
	...
	public void vertex(float x, float y) {
		...
		int index = pts - 1;
		color(colours[(index * 4)], colours[(index * 4) + 1],
		...
		if (lineCaps) {
			...
			if ((dx != 0) || (dy != 0)) {
				...
				for (int i = 0; i < 180 + step; i += step) {
					...
					GL.glVertex2f(
							points[0] + ((float) (Math.cos(ang) * width)),
		...
		if (lineCaps) {
			...
			if ((dx != 0) || (dy != 0)) {
				...
				for (int i = 0; i < 180 + step; i += step) {
					...
					GL.glVertex2f(
							points[(count * 2) - 2]
									+ ((float) (Math.cos(ang) * width)),
							points[(count * 2) - 1]
									+ ((float) (Math.sin(ang) * width)));
				}
				GL.glEnd();
			}
		}
	...
	private void bindColor(int index) {
		if (index < cpt) {
			if (renderHalf) {
				GL.glColor4f(colours[(index * 4)] * 0.5f,
						colours[(index * 4) + 1] * 0.5f,
						colours[(index * 4) + 2] * 0.5f,
