...
public class LineProcessor implements ElementProcessor {
	 ...
	 *             Indicates an invalid token in the path
	 */
	private static int processPoly(Polygon poly, Element element,
			StringTokenizer tokens) throws ParsingException {
		int count = 0;
		...
		while (tokens.hasMoreTokens()) {
			...
			try {
				...
				throw new ParsingException(element.getAttribute("id"),
						"Invalid token in points list", e);
			}
		}
