...
public class SVGMorph extends Diagram {
	 ...
	 * 
	 * @param diagram
	 *            The base diagram which provides the first step of the morph
	 */
	public SVGMorph(Diagram diagram) {
		...
		for (int i = 0; i < diagram.getFigureCount(); i++) {
			...
			Figure copy = new Figure(figure.getType(), new MorphShape(
					figure.getShape()), figure.getData(), figure.getTransform());
	 ...
	 * 
	 * @param diagram
	 *            The diagram to add as the next step in the morph
	 */
	 ...
	 * updateMorphTime() but can be used for smooth transitions between morphs.
	 * 
	 * @param diagram
	 *            The diagram to use as the base of the morph
	 */
	public void setExternalDiagram(Diagram diagram) {
		for (int i = 0; i < figures.size(); i++) {
			...
			for (int j = 0; j < diagram.getFigureCount(); j++) {
				...
				if (newBase.getData().getMetaData()
						.equals(figure.getData().getMetaData())) {
	 ...
	 * 
	 * @param delta
	 *            The amount to update the morph by
	 */
	...
	/**
	 * Set the "time" index for this morph. This is given in terms of diagrams,
	 * so 0.5f would give you the position half way between the first and second
	 * diagrams.
	 * 
	 * @param time
	 *            The time index to represent on this diagrams
	 */
