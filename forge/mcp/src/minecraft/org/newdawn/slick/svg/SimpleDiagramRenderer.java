...
/**
 * A very primtive implementation for rendering a diagram. This simply sticks
 * the shapes on the screen in the right fill and stroke colours
 * 
 * @author kevin
 */
public class SimpleDiagramRenderer {
	 ...
	 * 
	 * @param g
	 *            The graphics context to which we should render the diagram
	 */
		...
		for (int i = 0; i < diagram.getFigureCount(); i++) {
			...
			if (figure.getData().isFilled()) {
				if (figure.getData().isColor(NonGeometricData.FILL)) {
					g.setColor(figure.getData().getAsColor(
							NonGeometricData.FILL));
				...
				String fill = figure.getData().getAsReference(
						NonGeometricData.FILL);
				...
				if (diagram.getGradient(fill) != null) {
					...
					if (gradient.isRadial()) {
						fg = new RadialGradientFill(shape, diagram.getFigure(i)
								.getTransform(), gradient);
					} else {
						fg = new LinearGradientFill(shape, diagram.getFigure(i)
								.getTransform(), gradient);
					}
			...
			if (figure.getData().isStroked()) {
				if (figure.getData().isColor(NonGeometricData.STROKE)) {
					g.setColor(figure.getData().getAsColor(
							NonGeometricData.STROKE));
					g.setLineWidth(figure.getData().getAsFloat(
							NonGeometricData.STROKE_WIDTH));
					g.setAntiAlias(true);
					g.draw(diagram.getFigure(i).getShape());
					g.setAntiAlias(false);
					g.resetLineWidth();
