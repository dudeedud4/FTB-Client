...
public class NonGeometricData {
	 ...
	 * 
	 * @param metaData
	 *            The meta data (either label or id) for the figure
	 */
	...
	/**
	 * Get the meta data assigned to the figure. Either the label or the id
	 * value.
	 * 
	...
	/**
	 * Get an attribute value converted to a color. isColor should first be
	 * checked
	 * 
	 * @param attribute
	...
	public Color getAsColor(String attribute) {
		if (!isColor(attribute)) {
			throw new RuntimeException("Attribute " + attribute
					+ " is not specified as a color:" + getAttribute(attribute));
		}
	...
	public float getAsFloat(String attribute) {
		...
		try {
			...
			throw new RuntimeException("Attribute " + attribute
					+ " is not specified as a float:" + getAttribute(attribute));
		}
	}
	...
	public boolean isStroked() {
		return isColor(NonGeometricData.STROKE)
				&& (getAsFloat(NonGeometricData.STROKE_WIDTH) > 0);
	}
}
