...
public class ParticleSystem {
	...
	private class ParticlePool {
		...
		/**
		 * The list of particles left to be used, if this size() == 0 then the
		 * particle engine was too small for the effect
		 */
	 ...
	 * void: this is now sorted by emitters to allow emitter specfic state to be
	 * set for each emitter. actually this is used to allow setting an
	 * individual blend mode for each emitter
	 */
	...
	/**
	 * Create a particle specific to this system, override for your own
	 * implementations. These particles will be cached and reused within this
	 * system.
	 * 
	 * @param system
	...
	/**
	 * Set the position at which this system should render relative to the
	 * current graphics context setup
	 * 
	 * @param x
	 ...
	 * 
	 * @param x
	 *            The x coordinate to render the particle system at (in the
	 *            current coordinate space)
	 * @param y
	 *            The y coordinate to render the particle system at (in the
	 *            current coordiante space)
	 */
	public void render(float x, float y) {
		...
		for (int emitterIdx = 0; emitterIdx < emitters.size(); emitterIdx++) {
			// get emitter
			ParticleEmitter emitter = (ParticleEmitter) emitters
					.get(emitterIdx);
			...
			// now get the particle pool for this emitter and render all
			// particles that are in use
	...
	public void release(Particle particle) {
		if (particle != dummy) {
			ParticlePool pool = (ParticlePool) particlesByEmitter.get(particle
					.getEmitter());
	...
	/**
	 * Create a duplicate of this system. This would have been nicer as a
	 * different interface but may cause to much API change headache. Maybe next
	 * full version release it should be rethought.
	 * 
	 * TODO: Consider refactor at next point release
	 * 
	 * @return A copy of this particle system
	 * @throws SlickException
	 *             Indicates a failure during copy or a invalid particle system
	 *             to be duplicated
	 */
		...
		for (int i = 0; i < emitters.size(); i++) {
			if (!(emitters.get(i) instanceof ConfigurableEmitter)) {
				throw new SlickException(
						"Only systems contianing configurable emitters can be duplicated");
			}
		}
