...
public class RoundedRectangle extends Rectangle {
	...
	public static final int ALL = TOP_LEFT | TOP_RIGHT | BOTTOM_RIGHT
			| BOTTOM_LEFT;
	 ...
	 */
	public RoundedRectangle(float x, float y, float width, float height,
			float cornerRadius, int segmentCount) {
	...
	protected void createPoints() {
		...
		if (cornerRadius == 0) {
			...
			// the outer most set of points for each arc will also ac as the
			// points that start the
			// straight sides, so the straight sides do not have to be added.

			// top left corner arc
			if ((cornerFlags & TOP_LEFT) != 0) {
				tempPoints.addAll(createPoints(segmentCount, cornerRadius, x
						+ cornerRadius, y + cornerRadius, 180, 270));
			} else {
			...
			if ((cornerFlags & TOP_RIGHT) != 0) {
				tempPoints.addAll(createPoints(segmentCount, cornerRadius, x
						+ useWidth - cornerRadius, y + cornerRadius, 270, 360));
			} else {
			...
			if ((cornerFlags & BOTTOM_RIGHT) != 0) {
				tempPoints.addAll(createPoints(segmentCount, cornerRadius, x
						+ useWidth - cornerRadius,
						y + useHeight - cornerRadius, 0, 90));
			} else {
				tempPoints.add(new Float(x + useWidth));
			...
			if ((cornerFlags & BOTTOM_LEFT) != 0) {
				tempPoints.addAll(createPoints(segmentCount, cornerRadius, x
						+ cornerRadius, y + useHeight - cornerRadius, 90, 180));
			} else {
				tempPoints.add(new Float(x));
				tempPoints.add(new Float(y + useHeight));
			}
	 ...
	 */
	private List createPoints(int numberOfSegments, float radius, float cx,
			float cy, float start, float end) {
		ArrayList tempPoints = new ArrayList();
