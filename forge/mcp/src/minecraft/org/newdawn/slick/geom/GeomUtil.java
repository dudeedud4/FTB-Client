...
public class GeomUtil {
	...
	/**
	 * The maximum number of points returned by an operation - prevents full
	 * lockups
	 */
	...
	public Shape[] subtract(Shape target, Shape missing) {
		...
		for (int i = 0; i < missing.getPointCount(); i++) {
			if (target.contains(missing.getPoint(i)[0], missing.getPoint(i)[1])) {
				if (!onPath(target, missing.getPoint(i)[0],
						missing.getPoint(i)[1])) {
					found++;
				}
			}
		}
		for (int i = 0; i < target.getPointCount(); i++) {
			if (missing.contains(target.getPoint(i)[0], target.getPoint(i)[1])) {
				if (!onPath(missing, target.getPoint(i)[0],
						target.getPoint(i)[1])) {
					found++;
				}
			}
	...
	/**
	 * Join to shapes together. Note that the shapes must be touching for this
	 * method to work.
	 * 
	 * @param target
	...
	public Shape[] union(Shape target, Shape other) {
		...
		for (int i = 0; i < target.getPointCount(); i++) {
			if (other.contains(target.getPoint(i)[0], target.getPoint(i)[1])) {
				if (!other.hasVertex(target.getPoint(i)[0],
						target.getPoint(i)[1])) {
					touches = true;
					break;
				}
		...
		for (int i = 0; i < other.getPointCount(); i++) {
			if (target.contains(other.getPoint(i)[0], other.getPoint(i)[1])) {
				if (!target.hasVertex(other.getPoint(i)[0],
						other.getPoint(i)[1])) {
					touches = true;
					break;
				}
	...
	private Shape[] combine(Shape target, Shape other, boolean subtract) {
		if (subtract) {
			...
			// remove any points that are contianed in the shape we're removing,
			// these
			// are implicitly used
			for (int i = 0; i < target.getPointCount(); i++) {
			...
			for (int i = 0; i < target.getPointCount(); i++) {
				if (!other.contains(target.getPoint(i)[0],
						target.getPoint(i)[1])) {
					if (!other.hasVertex(target.getPoint(i)[0],
							target.getPoint(i)[1])) {
	 ...
	 * @param subtract
	 *            True if we should subtract missing from target, otherwise
	 *            union
	 * @param start
	 *            The point to start at
	 * @return The newly created shape
	 */
	private Shape combineSingle(Shape target, Shape missing, boolean subtract,
			int start) {
		...
		while (!poly.hasVertex(px, py) || (first) || (current != target)) {
			...
			// if the line between the current point and the next one intersect
			// the
			// other shape work out where on the other shape and start
			// traversing it's
			// path instead
			Line line = getLine(current, px, py,
					rationalPoint(current, point + dir));
			...
			if (hit != null) {
				...
				if (current.contains(pt.x + dx, pt.y + dy)) {
					// the point is the next one, we need to take the first and
					// traverse
					// the path backwards
					if (subtract) {
						if (current == missing) {
