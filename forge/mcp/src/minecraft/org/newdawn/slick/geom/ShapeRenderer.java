 ...
 * @author Mark Bernard
 * 
 *         Use this class to render shpaes directly to OpenGL. Allows you to
 *         bypass the Graphics class.
 */
public final class ShapeRenderer {
	...
	/**
	 * Draw the outline of the given shape. Only the vertices are set. The
	 * colour has to be set independently of this method.
	 * 
	 * @param shape
	 *            The shape to draw.
	 */
	...
	/**
	 * Draw the the given shape filled in. Only the vertices are set. The colour
	 * has to be set independently of this method.
	 * 
	 * @param shape
	 *            The shape to fill.
	 */
	...
	/**
	 * Draw the the given shape filled in with a texture. Only the vertices are
	 * set. The colour has to be set independently of this method.
	 * 
	 * @param shape
	...
	/**
	 * Draw the the given shape filled in with a texture. Only the vertices are
	 * set. The colour has to be set independently of this method. This method
	 * is required to fit the texture once across the shape.
	 * 
	 * @param shape
	 *            The shape to texture.
	 * @param image
	 ...
	 */
	public static final void texture(Shape shape, final Image image,
		...
		fill(shape, new PointCallback() {
			public float[] preRenderPoint(Shape shape, float x, float y) {
				...
				ty = image.getTextureOffsetY()
						+ (image.getTextureHeight() * ty);
	 ...
	 * Draw the the given shape filled in with a texture. Only the vertices are
	 * set. The colour has to be set independently of this method. This method
	 * is required to fit the texture scaleX times across the shape and scaleY
	 ...
	 *            The scale to apply on the y axis for texturing
	 */
	public static final void textureFit(Shape shape, final Image image,
			final float scaleX, final float scaleY) {
		if (!validFill(shape)) {
			return;
		}
	...
	public static final void texture(final Shape shape, final Image image,
			final float scaleX, final float scaleY, final ShapeFill fill) {
		if (!validFill(shape)) {
			return;
		}
	 ...
	 *            The image to tile across the shape
	 * @param gen
	 *            The texture coordinate generator to create coordiantes for the
	 *            shape
	 */
	public static final void texture(final Shape shape, Image image,
			final TexCoordGenerator gen) {
		Texture t = TextureImpl.getLastBind();
