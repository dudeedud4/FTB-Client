...
/**
 * A 2 dimensional transformation that can be applied to <code>Shape</code>
 * implemenations.
 * 
 * @author Mark
 */
public class Transform {
	...
	public Transform(float matrixPosition[]) {
		if (matrixPosition.length != 6) {
			throw new RuntimeException(
					"The parameter must be a float array of length 6.");
		}
		this.matrixPosition = new float[] { matrixPosition[0],
				matrixPosition[1], matrixPosition[2], matrixPosition[3],
				matrixPosition[4], matrixPosition[5], 0, 0, 1 };
	}
	 ...
	 */
	public Transform(float point00, float point01, float point02,
			float point10, float point11, float point12) {
		matrixPosition = new float[] { point00, point01, point02, point10,
				point11, point12, 0, 0, 1 };
	}

	/**
	 * Transform the point pairs in the source array and store them in the
	 * destination array. All operations will be done before storing the results
	 * in the destination. This way the source and destination array can be the
	 * same without worry of overwriting information before it is transformed.
	 * 
	 * @param source
	...
	public void transform(float source[], int sourceOffset,
		...
		// TODO performance can be improved by removing the safety to the
		// destination array
		float result[] = new float[numberOfPoints * 2];

		for (int i = 0; i < numberOfPoints * 2; i += 2) {
			for (int j = 0; j < 6; j += 3) {
				result[i + (j / 3)] = source[i + sourceOffset]
						* matrixPosition[j] + source[i + sourceOffset + 1]
						* matrixPosition[j + 1] + 1 * matrixPosition[j + 2];
			}
		}
		// for safety of the destination, the results are copied after the
		// entire operation.
		for (int i = 0; i < numberOfPoints * 2; i += 2) {
	...
	public Transform concatenate(Transform tx) {
		...
		float n00 = matrixPosition[0] * tx.matrixPosition[0]
				+ matrixPosition[1] * tx.matrixPosition[3];
		float n01 = matrixPosition[0] * tx.matrixPosition[1]
				+ matrixPosition[1] * tx.matrixPosition[4];
		float n02 = matrixPosition[0] * tx.matrixPosition[2]
				+ matrixPosition[1] * tx.matrixPosition[5] + matrixPosition[2];
		float n10 = matrixPosition[3] * tx.matrixPosition[0]
				+ matrixPosition[4] * tx.matrixPosition[3];
		float n11 = matrixPosition[3] * tx.matrixPosition[1]
				+ matrixPosition[4] * tx.matrixPosition[4];
		float n12 = matrixPosition[3] * tx.matrixPosition[2]
				+ matrixPosition[4] * tx.matrixPosition[5] + matrixPosition[5];
		mp[0] = n00;
		mp[1] = n01;
		mp[2] = n02;
		mp[3] = n10;
		...
		mp[5] = n12;
		//
		// mp[0] = matrixPosition[0] * transform.matrixPosition[0] +
		// matrixPosition[0] * transform.matrixPosition[3] + matrixPosition[0] *
		// transform.matrixPosition[6];
		// mp[1] = matrixPosition[1] * transform.matrixPosition[1] +
		// matrixPosition[1] * transform.matrixPosition[4] + matrixPosition[1] *
		// transform.matrixPosition[7];
		// mp[2] = matrixPosition[2] * transform.matrixPosition[2] +
		// matrixPosition[2] * transform.matrixPosition[5] + matrixPosition[2] *
		// transform.matrixPosition[8];
		// mp[3] = matrixPosition[3] * transform.matrixPosition[0] +
		// matrixPosition[3] * transform.matrixPosition[3] + matrixPosition[3] *
		// transform.matrixPosition[6];
		// mp[4] = matrixPosition[4] * transform.matrixPosition[1] +
		// matrixPosition[4] * transform.matrixPosition[4] + matrixPosition[4] *
		// transform.matrixPosition[7];
		// mp[5] = matrixPosition[5] * transform.matrixPosition[2] +
		// matrixPosition[5] * transform.matrixPosition[5] + matrixPosition[5] *
		// transform.matrixPosition[8];
		//
		matrixPosition = mp;
		return this;
	...
	public static Transform createRotateTransform(float angle) {
		return new Transform((float) FastTrig.cos(angle),
				-(float) FastTrig.sin(angle), 0, (float) FastTrig.sin(angle),
				(float) FastTrig.cos(angle), 0);
	}
