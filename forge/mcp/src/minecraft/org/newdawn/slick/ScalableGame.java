...
/**
 * A wrapper to allow any game to be scalable. This relies on knowing the normal
 * width/height of the game - i.e. the dimensions that the game is expecting to
 * be run at. The wrapper then takes the size of the container and scales
...
public class ScalableGame implements Game {
	...
	public void recalculateScale() throws SlickException {
		...
		if (maintainAspect) {
			boolean normalIsWide = (normalWidth / normalHeight > 1.6 ? true
					: false);
