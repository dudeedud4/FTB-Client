 ...
 * maximum texture size supported by the card. In most cases it makes sense to
 * make sure all of your image resources are less than 512x512 in size when
 ...
 * 
 * TODO: The code in here isn't pretty, really needs revisiting with a comment
 * stick.
 * 
 * @author kevin
 */
public class BigImage extends Image {
	 ...
	 *             Indicates we were unable to locate the resource
	 */
	...
	private void build(String ref, int filter, int tileSize)
			throws SlickException {
		try {
			final LoadableImageData data = ImageDataFactory
					.getImageDataFor(ref);
			final ByteBuffer imageBuffer = data.loadImage(
	 ...
	 *            The maximum size of the tiles to use to build the bigger image
	 */
	private void build(final LoadableImageData data,
			final ByteBuffer imageBuffer, int filter, int tileSize) {
		...
		for (int x = 0; x < xcount; x++) {
			for (int y = 0; y < ycount; y++) {
				...
				final ByteBuffer subBuffer = BufferUtils
						.createByteBuffer(tileSize * tileSize * components);
	...
	/**
	 * @see org.newdawn.slick.Image#draw(float, float, float, float, float,
	 *      float, float, float)
	 */
	public void draw(float x, float y, float x2, float y2, float srcx,
		...
		Image subImage = getSubImage((int) srcx, (int) srcy, srcwidth,
				srcheight);
	...
	public void drawEmbedded(float x, float y, float width, float height) {
		...
		for (int tx = 0; tx < xcount; tx++) {
			...
			for (int ty = 0; ty < ycount; ty++) {
				...
				if ((lastBind == null)
						|| (image.getTexture() != lastBind.getTexture())) {
					if (lastBind != null) {
						lastBind.endUse();
					}
					lastBind = image;
				...
				image.drawEmbedded(xp + x, yp + y, image.getWidth(),
						image.getHeight());
	...
	public void ensureInverted() {
		throw new OperationNotSupportedException(
				"Doesn't make sense for tiled operations");
	}
	...
	public Color getColor(int x, int y) {
		throw new OperationNotSupportedException(
				"Can't use big images as buffers");
	}
	...
	public Image getFlippedCopy(boolean flipHorizontal, boolean flipVertical) {
		...
		if (flipHorizontal) {
			...
			for (int x = 0; x < xcount; x++) {
				for (int y = 0; y < ycount; y++) {
					image.images[x][y] = images[xcount - 1 - x][y]
		...
		if (flipVertical) {
			...
			for (int x = 0; x < xcount; x++) {
				for (int y = 0; y < ycount; y++) {
					image.images[x][y] = images[x][ycount - 1 - y]
	...
	public Graphics getGraphics() throws SlickException {
		throw new OperationNotSupportedException(
				"Can't use big images as offscreen buffers");
	}
	...
	public Image getSubImage(int x, int y, int width, int height) {
		...
		for (int xt = 0; xt < xcount; xt++) {
			...
			for (int yt = 0; yt < ycount; yt++) {
				...
				// we want or the bottom corrent of the image is, then consider
				// using the
				// image

				// this image contributes to the sub image we're attempt to
				// retrieve
				...
				if ((targetWidth > 0) && (targetHeight > 0)) {
					Image subImage = current.getSubImage((int) (targetX1 - xp),
							(int) (targetY1 - yp), (targetX2 - targetX1),
							(targetY2 - targetY1));
					foundStart = true;
	...
	/**
	 * Get a sub-image that builds up this image. Note that the offsets used
	 * will depend on the maximum texture size on the OpenGL hardware
	 * 
	 * @param offsetX
	...
	/**
	 * @see org.newdawn.slick.Image#draw(float, float, float, float, float,
	 *      float, float, float, org.newdawn.slick.Color)
	 */
	public void draw(float x, float y, float x2, float y2, float srcx,
	...
	/**
	 * @see org.newdawn.slick.Image#drawEmbedded(float, float, float, float,
	 *      float, float, float, float, org.newdawn.slick.Color)
	 */
