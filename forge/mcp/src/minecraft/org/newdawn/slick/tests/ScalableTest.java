...
public class ScalableTest extends BasicGame {
	...
	public void render(GameContainer container, Graphics g)
		...
		g.drawString(container.getInput().getMouseX() + ","
				+ container.getInput().getMouseY(), 10, 400);
		g.setColor(Color.red);
		g.fillOval(container.getInput().getMouseX() - 10, container.getInput()
				.getMouseY() - 10, 20, 20);
	}
	...
	public static void main(String[] argv) {
		...
		// ScalableGame(new InputTest(),600,600));
		...
		// try {
		// AppGameContainer container = new AppGameContainer(new
		// ScalableGame(new InputTest(),600,600,true));
		...
		try {
			ScalableGame game = new ScalableGame(new ScalableTest(), 1024, 568,
					true) {
				...
				protected void renderOverlay(GameContainer container, Graphics g) {
					...
					g.drawString(container.getInput().getMouseX() + ","
							+ container.getInput().getMouseY(), 400, 20);
				}
