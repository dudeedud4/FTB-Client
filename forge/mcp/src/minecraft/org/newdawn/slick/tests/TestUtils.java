...
public class TestUtils {
	...
	public void init() {
		...
		// texture load, the second argument is a name assigned to the texture
		// to
		// allow for caching in the texture loader. The 3rd argument indicates
		// whether
		...
		try {
			...
			// a sound
			oggEffect = AudioLoader.getAudio("OGG", new FileInputStream(
					"testdata/restart.ogg"));
			...
			// worked
			oggStream = AudioLoader.getStreamingAudio("OGG", new File(
					"testdata/bongos.ogg").toURL());

			// can load mods (XM, MOD) using ibxm which is then played through
			// OpenAL. MODs
			// are always streamed based on the way IBXM works
			modStream = AudioLoader.getStreamingAudio("MOD", new File(
					"testdata/SMB-X.XM").toURL());
			...
			// a sound
			aifEffect = AudioLoader.getAudio("AIF", new FileInputStream(
					"testdata/burp.aif"));

			// you can play wavs by loading the complete thing into
			// a sound
			wavEffect = AudioLoader.getAudio("WAV", new FileInputStream(
					"testdata/cbrown01.wav"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
