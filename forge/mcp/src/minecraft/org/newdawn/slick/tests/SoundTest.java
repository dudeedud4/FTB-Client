...
public class SoundTest extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		try {
			engine = AudioLoader.getAudio("WAV",
					ResourceLoader.getResourceAsStream("testdata/engine.wav"));
		} catch (IOException e) {
	...
	public void render(GameContainer container, Graphics g) {
		...
		g.drawString("The OGG loop is now streaming from the file, woot.", 100,
				60);
		...
		g.drawString("Press Y or X to change individual volume of music", 100,
				300);
		g.drawString("Press N or M to change global volume of sound fx", 100,
				330);
		g.setColor(Color.blue);
		g.drawString(
				"Global Sound Volume Level: " + container.getSoundVolume(),
				150, 390);
		g.drawString(
				"Global Music Volume Level: " + container.getMusicVolume(),
				150, 420);
		g.drawString("Current Music Volume Level: " + music.getVolume(), 150,
				450);
	}
