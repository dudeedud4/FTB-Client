...
public class ShapeTest extends BasicGame {
	...
	public void createPoly(float x, float y) {
		...
		randomShape.addPoint(0 + (int) (Math.random() * change),
				0 + (int) (Math.random() * change));
		randomShape.addPoint(size - (int) (Math.random() * change),
				0 + (int) (Math.random() * change));
		randomShape.addPoint(size - (int) (Math.random() * change), size
				- (int) (Math.random() * change));
		randomShape.addPoint(0 + (int) (Math.random() * change), size
				- (int) (Math.random() * change));
	...
	public void init(GameContainer container) throws SlickException {
		...
		shapes.add(circle);
		polygon = new Polygon(new float[] { 550, 10, 600, 40, 620, 100, 570,
				130 });
		shapes.add(polygon);
