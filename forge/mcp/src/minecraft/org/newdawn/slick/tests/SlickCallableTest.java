...
/**
 * A test for slick callables giving the chance to perform normal GL in mid
 * Slick render
 * 
 * @author kevin
 */
public class SlickCallableTest extends BasicGame {
	...
	/**
	 * Render the GL scene, this isn't efficient and if you know OpenGL I'm
	 * assuming you can see why. If not, you probably don't want to use this
	 * feature anyway
	 */
	 ...
	 * 
	 * @param inner_radius
	 *            The inner radius of the gear
	 * @param outer_radius
	 *            The outer radius of the gear
	 * @param width
	 *            The width/depth of the gear
	 * @param teeth
	 *            The number of teeth
	 * @param tooth_depth
	...
	private void gear(float inner_radius, float outer_radius, float width,
		...
		for (i = 0; i <= teeth; i++) {
			...
			GL11.glVertex3f(r0 * (float) Math.cos(angle),
					r0 * (float) Math.sin(angle), width * 0.5f);
			GL11.glVertex3f(r1 * (float) Math.cos(angle),
					r1 * (float) Math.sin(angle), width * 0.5f);
			if (i < teeth) {
				GL11.glVertex3f(r0 * (float) Math.cos(angle),
						r0 * (float) Math.sin(angle), width * 0.5f);
				GL11.glVertex3f(r1 * (float) Math.cos(angle + 3.0f * da), r1
		...
		for (i = 0; i < teeth; i++) {
			...
			GL11.glVertex3f(r1 * (float) Math.cos(angle),
					r1 * (float) Math.sin(angle), width * 0.5f);
			GL11.glVertex3f(r2 * (float) Math.cos(angle + da), r2
					* (float) Math.sin(angle + da), width * 0.5f);
			GL11.glVertex3f(r2 * (float) Math.cos(angle + 2.0f * da), r2
		...
		for (i = 0; i <= teeth; i++) {
			...
			GL11.glVertex3f(r0 * (float) Math.cos(angle),
					r0 * (float) Math.sin(angle), -width * 0.5f);
			GL11.glVertex3f(r1 * (float) Math.cos(angle + 3 * da), r1
		...
		for (i = 0; i < teeth; i++) {
			...
			GL11.glVertex3f(r1 * (float) Math.cos(angle + 3 * da), r1
					* (float) Math.sin(angle + 3 * da), -width * 0.5f);
			GL11.glVertex3f(r2 * (float) Math.cos(angle + 2 * da), r2
		...
		for (i = 0; i < teeth; i++) {
			...
			GL11.glVertex3f(r2 * (float) Math.cos(angle + da), r2
					* (float) Math.sin(angle + da), -width * 0.5f);
			GL11.glNormal3f((float) Math.cos(angle), (float) Math.sin(angle),
					0.0f);
			GL11.glVertex3f(r2 * (float) Math.cos(angle + 2 * da), r2
					* (float) Math.sin(angle + 2 * da), width * 0.5f);
			GL11.glVertex3f(r2 * (float) Math.cos(angle + 2 * da), r2
		...
		GL11.glVertex3f(r1 * (float) Math.cos(0), r1 * (float) Math.sin(0),
				width * 0.5f);
		GL11.glVertex3f(r1 * (float) Math.cos(0), r1 * (float) Math.sin(0),
				-width * 0.5f);
		GL11.glEnd();
