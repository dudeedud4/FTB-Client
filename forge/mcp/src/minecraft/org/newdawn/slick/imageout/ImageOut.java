...
/**
 * A static hook to access all the Image output utilities. The list of format
 * strings provided is not the limit of capability. These are provided for
 * utility, use @see {@link #getSupportedFormats()} for a full list of supported
 * formats.
 * 
 * @author kevin
 */
public class ImageOut {
	 ...
	 * 
	 * @param image
	 *            The image to write out to
	 * @param format
	 *            The format to write the image out in
	 * @param out
	 *            The output stream to which the image should be written
	 * @throws SlickException
	 *             Indicates a failure to write the image in the specified
	 *             format
	 */
	 ...
	 *            The output stream to which the image should be written
	 * @param writeAlpha
	 *            True if we should write the alpha channel out (some formats
	 *            don't support this, like JPG)
	 * @throws SlickException
	 *             Indicates a failure to write the image in the specified
	 *             format
	 */
	public static void write(Image image, String format, OutputStream out,
			boolean writeAlpha) throws SlickException {
		try {
			ImageWriter writer = ImageWriterFactory.getWriterForFormat(format);
			writer.saveImage(image, format, out, writeAlpha);
		} catch (IOException e) {
			throw new SlickException(
					"Unable to write out the image in format: " + format, e);
		}
	}

	/**
	 * Write an image out to a file on the local file system. The format of the
	 * output is determined based on the file name extension
	 * 
	 * @param image
	 ...
	 * @throws SlickException
	 *             Indicates a failure to write the image in the determined
	 *             format
	 */
	...
	public static void write(Image image, String dest, boolean writeAlpha)
		...
		try {
			...
			if (ext < 0) {
				throw new SlickException("Unable to determine format from: "
						+ dest);
			}
			...
			throw new SlickException("Unable to write to the destination: "
					+ dest, e);
		}
	}
